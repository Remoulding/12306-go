<template>
  <Card>
    <Space>
      <Input v-model:value="state.searchName" placeholder="请输入乘客姓名" allow-clear></Input>
      ...
      <Button danger :disabled="!state.selectedRowKeys.length" @click="patchDelete">
        <template #icon>
          <DeleteOutlined />
        </template>
        批量删除</Button>
    </Space>
    <Table :loading="loading" :row-selection="{
      selectedRowKeys: state.selectedRowKeys,
      onChange: onSelectChange
    }" :style="{ marginTop: '20px' }" :columns="column" :data-source="passengerList" :pagination="false">
      <template #idType="{ text }">
        <span>{{ text === 0 ? '中国居民身份证' : '其他' }}</span></template>
      <template #edit="{ text, record }">
        <span @click="() => getDeletePassenger(record.id)"
          :style="{ marginRight: '20px', color: 'red', cursor: 'pointer' }">
          <DeleteOutlined />
        </span>
        <span @click="() => router.push(`/addPassenger?type=edit&id=${record.id}`)" :style="{ cursor: 'pointer' }">
          <EditOutlined />
        </span></template>
    </Table>
  </Card>
